---
title: "R Programming Basics - Session 1"
description: |
...
*This content was presented to Nelson\\Nygaard Staff at a Lunch and Learn webinar on Wednesday, February 3rd, 2021, and is [available as a recording here](https://web.microsoftstream.com/video/2eb9d408-3723-4179-b8b8-eb1641b2a40a) and embedded below.*

<p align="center"><iframe width="640" height="360" src="https://web.microsoftstream.com/video/2eb9d408-3723-4179-b8b8-eb1641b2a40a?autoplay=false&amp;showinfo=true" allowfullscreen style="border:none;"></iframe></p>

# Today's Agenda

- Announcements (5 min)
- R Data Structures (55 min)

# Announcements

## Welcome

Welcome to the inaugural session of a new ongoing round of R trainings. These trainings will be happening on a biweekly basis for the forseeable future, on Wednesdays at 12 pm Pacific time. The draft schedule (subject to topic change based on feedback received) is available [here](https://perkinsandwill.github.io/nn_r_training/user-resources/2021-01-23-lnl-schedule/). 

Approximately every fourth session will be a work share session, in which multiple people will be lined up prior to the session to share some interesting R-reliant project they have been working on. Please contact [Bryan](mailto:bblanc@nelsonnygaard.com) if you would like to present something at the first scheduled work share session on March 17th, 2021. 

Occasionally, if Bryan has an especially busy project load, is taking vacation, there is an intersecting NN holiday, or there is a lack of topics, these will be canceled or replaced with a free form work share session. Advanced notice will be provided for these occasions.  Please provide feedback on the schedule if you would like a topic scheduled sooner or would like to add additional topics by contacting [Bryan directly](mailto:bblanc@nelsonnygaard.com).

## R 1-on-1 Training Program

I want to publicize here that my time (and likely others in the future) will be available for 1-on-1 help by request through [a form](https://forms.gle/mMU2Pt3TRAoEnntPA) to help systematize the process. The form will always be available on the home page of the [R Training Website](https://forms.gle/mMU2Pt3TRAoEnntPA) and in the topic of the #r-users Slack channel. If you think your question is going to be a quick response, feel free to just post it to the #r-users Slack channel, and there is no need to submit a form entry. If you think you want my time in 30 minute increments to help you on a project (or something non-billable, I have some overhead time available), please fill out the form so I can understand what the demand for this would be as Itry to offer this in a more structured way. 

I'll announce any future changes to this program both in these sessions as well in the #r-users Slack channel. 

# R Data Structures

This first session focuses solely on R data structures, while the next will delve in to control flows, functions, and iteration. Future sessions will dive deeper into specific data structures, like character strings and date/time objects. 
...
You are probably most familiar with data frames (or their modernized analogue, tibbles) and the packages that work with them. But as you start to write your own functions, and dig deeper into R, you need to learn about **vectors**, the objects that underlie tibbles. If you've learned R in a more traditional way, you're probably already familiar with vectors, as most R resources start with vectors and work their way up to data frames/tibbles. Vectors are particularly important as most of the functions you will write will work with vectors. 
...
The chief difference between atomic vectors and lists is that atomic vectors are __homogeneous__, while lists can be __heterogeneous__. There's one other related object: `NULL`. `NULL` is often used to represent the absence of a vector (as opposed to `NA` which is used to represent the absence of a value in a vector). `NULL` typically behaves like a vector of length 0. Figure \@ref(fig:datatypes) summarizes the interrelationships. 
...
Vectors can also contain arbitrary additional metadata in the form of attributes. These attributes are used to create __augmented vectors__ which build on additional behavior. There are three important types of augmented vector:
...
Sometimes you want to do different things based on the type of vector. One option is to use `typeof()`. Another is to use a test function which returns a `TRUE` or `FALSE`. Base R provides many functions like `is.vector()` and `is.atomic()`, but they often return surprising results. Instead, it's safer to use the `is_*` functions provided by purrr, which are summarized in the table below.
...
You have probably used `dplyr::filter()` to filter the rows in a tibble. `filter()` only works with tibble, so we'll need different tool for vectors: `[`. `[` is the subsetting function, and is called like `x[a]`. There are four types of things that you can subset a vector with:
...
You've seen names above, and we will cover matrices in the next section. It remains to describe the class, which controls how __generic functions__ work. Generic functions are key to object oriented programming in R, because they make functions behave differently for different classes of input. A detailed discussion of object oriented programming is beyond the scope of this module, but you can read more about it in _Advanced R_ at <http://adv-r.had.co.nz/OO-essentials.html#s3>.
...
Adding a `dim` attribute to a vector allows it to behave like a 2-dimensional __matrix__ or a multi-dimensional __array__. Matrices and arrays are primarily mathematical and statistical tools, not programming tools, so they'll be used infrequently and only covered briefly here. 
...
You can subset arrays in multiple dimensions. For a two dimensional matrix, below is an example of how we would select specific elements in the matrix. 

```{r}
ex_matrix = matrix(1:9,3,3)
print(ex_matrix)

upper_right = ex_matrix[1:2,2:3]
print(upper_right)

lower_left = ex_matrix[2:3,1:2]
print(lower_left)
...
That's it! There are of course more things to learn about R data structures as you become more experienced in using R -- some additional reference materials are provided below. However, this should serve as a good base of knowledge to refer back to as we delve into more advanced topics in future modules. As mentioned, the next module will cover several other foundational aspects of programming with R -- control flows (e.g., if statements), functions, and iteration (e.g., loops). 
...
*This content was presented to Nelson\\Nygaard Staff at a Lunch and Learn webinar on Wednesday, February 3rd, and is [available as a recording here](https://web.microsoftstream.com/video/2eb9d408-3723-4179-b8b8-eb1641b2a40a) and embedded at the top of the page.*
